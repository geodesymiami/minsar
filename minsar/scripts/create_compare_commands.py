#!/usr/bin/env python3
"""Write a bash driver to set up SAFE, CSLC, DISP-S1, and ISCE2/miaplpy compare runs."""
...
COMPARE_SUFFIXES = ("SAFE", "CSLC", "DISP", "ISCE2", "MIAPLPY")
ARGV_FIX_KW = {
    "consume_one": (
        "--flight-dir",
        "--start",
...
DESCRIPTION = (
    "Write a bash script with create_isce3_runfiles.py (--safe, --cslc, --disp-S1) "
    "and minsarApp.bash (--no-mintpy --miaplpy) for the same AOI and dates. "
    "Each pipeline uses a separate project name under $SCRATCHDIR."
)

EXAMPLE = """Examples:
  create_compare_commands.py 19.4:19.54,-155.02:-154.80 qqHawaiiPuna --flight-dir desc --start 20250101 --end 20250630
  create_compare_commands.py 19.4:19.54,-155.02:-154.80 qqHawaiiPuna --flight-dir desc --start 20250101 --end 20250630 --output qqHawaiiPuna_compare.bash
  create_compare_commands.py 19.4:19.54,-155.02:-154.80 qqHawaiiPuna --flight-dir desc --start 20250101 --end 20250630 --run
"""
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("name", help="project base name; SAFE/CSLC/DISP/ISCE2 suffixes are appended")
    ...
    parser.add_argument("--run", action="store_true", help="after writing the driver, run the three create_isce3_runfiles.py commands")
    return parser
...
def build_compare_script(
    ...
    *,
    ...
    lines = [
        "#!/usr/bin/env bash",
        "set -e",
        f"# Compare SAFE, CSLC, DISP-S1 (ISCE3), and MiaplPy (ISCE2) for {stem}",
        f"# Project dirs: $SCRATCHDIR/{names['safe']}, {names['cslc']}, {names['disp']}, {names['isce2']}",
        _isce3_command(aoi, names["safe"], flight_dir=flight_dir, start=start, end=end, data_flag="--safe", track=track, frame_id=frame_id),
        _isce3_command(aoi, names["cslc"], flight_dir=flight_dir, start=start, end=end, data_flag="--cslc", track=track, frame_id=frame_id),
        _isce3_command(aoi, names["disp"], flight_dir=flight_dir, start=start, end=end, data_flag="--disp-S1", track=track, frame_id=frame_id),
...
def main(iargs: list[str] | None = None) -> int:
    ...
    scratch = os.environ.get("SCRATCHDIR", "$SCRATCHDIR")
    print(body.rstrip())
    print(f"Wrote {out_path}")
    print(
        "Project dirs: "
        + ", ".join(f"{scratch}/{name}" for name in (names["safe"], names["cslc"], names["disp"], names["isce2"]))
    )

    if inps.run:
        import subprocess

        for data_flag, project in (
            ("--safe", names["safe"]),
            ("--cslc", names["cslc"]),
            ("--disp-S1", names["disp"]),
        ):
