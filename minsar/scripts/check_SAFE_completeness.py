...
def main(iargs=None):
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only report which SAFE dirs would be removed; do not remove or append to DATES_REMOVED.txt",
    )
    ...
    for safe_path in safe_dirs:
        ...
        if inps.dry_run:
            print(f"[dry-run] Would remove: {safe_path} ({incomplete_reason})")
            if date_str:
                print(f"  -> would append to {dates_removed_file}: {date_str}  {incomplete_reason}")
            continue
        if date_str:
