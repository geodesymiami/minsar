...
function print_help {
    ...
    echo ""
    echo "Subset radar-coded slcStack.h5 and geometryRadar.h5 with MintPy subset.py --lookup."
    echo ""
    echo "Examples:"
...
echo "Running....... subset.py slcStack.h5 --lookup geometryRadar.h5 --lat $LAT_MIN $LAT_MAX --lon $LON_MIN $LON_MAX"
echo "followed by... mv sub_slcStack.h5 ../../${OUTDIR}/inputs/slcStack.h5"
echo ""
subset.py slcStack.h5 --lookup geometryRadar.h5 --lat "$LAT_MIN" "$LAT_MAX" --lon "$LON_MIN" "$LON_MAX"
mv "sub_slcStack.h5" "../../${OUTDIR}/inputs/slcStack.h5"

echo "Running... subset.py geometryRadar.h5 --lookup geometryRadar.h5 --lat $LAT_MIN $LAT_MAX --lon $LON_MIN $LON_MAX"
echo "followed by... mv sub_geometryRadar.h5 ../../${OUTDIR}/inputs/geometryRadar.h5"
echo ""
subset.py geometryRadar.h5 --lookup geometryRadar.h5 --lat "$LAT_MIN" "$LAT_MAX" --lon "$LON_MIN" "$LON_MAX"
mv "sub_geometryRadar.h5" "../../${OUTDIR}/inputs/geometryRadar.h5"

cd -
echo "subset_sarvey.bash completed. Output files in $OUTDIR/inputs"
