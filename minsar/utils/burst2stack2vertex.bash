...
# Usage: burst2stack2vertex.bash [burst2stack] [options]
...
usage() {
    cat <<'EOF'
Usage: burst2stack2vertex.bash [burst2stack] --rel-orbit N --start-date DATE --end-date DATE --extent lon_min lat_min lon_max lat_max
...
Options:
  ...
  --keep-files         Ignored (burst2stack compatibility)
  --all-anns           Ignored (burst2stack compatibility)
  --rel-orbit N        Relative orbit number
...
Output:
  Full ASF Vertex URL to stdout. On macOS, also opens the URL in Safari (via system_utils.detect_operating_system).
  Map zoom is fixed at 9.1.

Examples:
  burst2stack2vertex.bash --rel-orbit 87 --start-date 2025-06-01 --end-date 2099-12-31 --extent -156.1 18.9 -154.5 19.9

  burst2stack2vertex.bash burst2stack --rel-orbit 87 --start-date 2025-01-01 --end-date 2025-12-31 --extent -86.6062 12.3516 -86.44 12.5019

  burst2stack2vertex.bash burst2stack --rel-orbit 87 --start-date 2025-01-01 --end-date 2025-12-31 --extent -156.1 18.9 -154.5 19.9 --keep-files --all-anns
...
while [[ $i -lt $# ]]; do
    ...
    case "$a" in
        ...
        --extent)
            ...
            ;;
        --keep-files|--all-anns)
            # Ignored for burst2stack command-line compatibility
            ;;
        *)
...
# Base URL and fixed params (zoom 9.1: fixed value for consistent map scale)
base="https://search.asf.alaska.edu/#/"
params="zoom=9.1&center=${lon_center},${lat_center}&polygon=${polygon_enc}&dataset=SENTINEL-1%20BURSTS&maxResults=250&resultsLoaded=true"
...
# On macOS, open in Safari (use system_utils.py for OS detection)
detected_os=""
repo_root="$(cd "$SCRIPT_DIR/../.." && pwd)"
if [[ -n "$repo_root" ]]; then
    detected_os=$(PYTHONPATH="$repo_root" python3 -c "
