...
if [[ -x tools/snaphu-v2.0.7/bin/snaphu ]]; then
    ...
    [[ "$(uname)" == "Linux" ]] && perl -pi -e 's/-arch\s+\S+\s+//g' tools/snaphu-v2.0.7/src/Makefile
...
### Adding ISCE fixes ###
if [[ "$(uname)" == "Linux" ]]; then
python_version="python$(tools/miniforge3/envs/minsar/bin/python -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')"
