...
For `minsarApp.bash` end-of-run reporting, `upload.log` and `insarmaps.log` are append-only history logs. The footer prints only URLs generated by the current invocation using line-count deltas (before/after each run), rather than printing fixed tail counts.

When `minsarApp.bash` orchestrates upload/ingest, it passes `--quiet-summary` to suppress intermediate child-script URL printing and keeps URL output consolidated in the final minsarApp footer. Direct standalone calls to `upload_data_products.py` and `ingest_insarmaps.bash` remain verbose by default.
