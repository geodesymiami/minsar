...
| `minsar/scripts/egms_download.bash` | ssara-style: search (AOI+level+releases) → filter → `--print` / `--download` |
| `minsar/scripts/egms_concat_csv.py` | Concatenate EGMS L2 CSV/zip bursts (geo sort W→E, N→S) |
