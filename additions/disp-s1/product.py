...
MinSAR patch (``additions/disp-s1/product.py``):
...
def _chunk_shape_for_array(shape: tuple[int, ...]) -> tuple[int, ...]:
    """Return HDF5 chunk shape capped by *shape* (MinSAR patch for small AOIs)."""
    if len(shape) == 0:
        return shape
    if len(shape) == 1:
        return (min(shape[0], CHUNK_SHAPE[0]),)
    *leading, ysize, xsize = shape
    spatial = (min(CHUNK_SHAPE[0], ysize), min(CHUNK_SHAPE[1], xsize))
    if not leading:
        return spatial
    lead_chunks = tuple(1 if dim > 1 else dim for dim in leading)
    return (*lead_chunks, *spatial)


def _hdf5_opts_for_array(data: ArrayLike) -> dict[str, Any]:
    """Copy default HDF5 options with chunk sizes valid for *data*."""
    arr = np.asarray(data)
    if arr.ndim < 2 or arr.size <= 1:
        return {}
    opts = HDF5_OPTS.copy()
    opts["chunks"] = _chunk_shape_for_array(arr.shape)
    return opts
...
def _create_dataset(
    *,
    ...
    if isinstance(data, str):
        ...
        options = _hdf5_opts_for_array(data)
