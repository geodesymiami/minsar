...
def main(iargs=None):
    ...
    # Copy XML file if it exists
    input_xml = inps.input_slc + '.xml'
    output_xml = inps.output_slc + '.xml'
    if os.path.exists(input_xml):
        cmd = 'cp ' + input_xml + ' ' + output_xml
        print(f'Copying XML file: {cmd}')
        os.system(cmd)
    else:
        print(f'Warning: XML file not found: {input_xml}')
