...
# MinSAR: Patched for 1-burst — check for empty bursts before indexing and raise clear error.
...
def main(iargs=None):
    ...
    for swath in swathList:
        ...
        # MinSAR 1-burst: overlap products (reference/overlap, secondary/overlap) have 0 bursts when only one burst exists.
        # Skip this swath so the run completes; main interferograms (no --overlap) use reference/IW*.xml and work.
        if inps.overlap and (not topReference.bursts or len(topReference.bursts) == 0 or not topCoreg.bursts or len(topCoreg.bursts) == 0):
            print(
                'Skipping overlap interferograms for IW{}: reference or secondary overlap has no bursts '
                '(1-burst stack has no overlap regions). Reference overlap: {} ; Secondary overlap: {}.'.format(
                    swath, inps.reference, inps.secondary
                )
            )
            continue

        # MinSAR 1-burst patch: avoid IndexError when product has no bursts (non-overlap path or missing files)
        if not topReference.bursts or len(topReference.bursts) == 0:
            raise RuntimeError(
                'Reference has no bursts (loaded from {}). '
                'For 1-burst stacks, ensure burst_01.slc (and burst_01.slc.xml) exist in reference/IW{}/. '
                'See architecture_docs/1-BURST_ISCE_TROUBLESHOOTING.md'.format(inps.reference, swath)
            )
        if not topCoreg.bursts or len(topCoreg.bursts) == 0:
            raise RuntimeError(
                'Secondary has no bursts (loaded from {}). '
                'For 1-burst stacks, ensure burst_01.slc (and burst_01.slc.xml) exist in coreg_secondarys/<date>/IW{}/. '
                'See architecture_docs/1-BURST_ISCE_TROUBLESHOOTING.md'.format(inps.secondary, swath)
            )
