...
def main(iargs=None):
    ...
    for swath in swathList:
        ...
        # MinSAR 1-burst: generateIgram skips overlap when no bursts; overlap ifg files are missing. Skip this swath.
        ifg_top = os.path.join(inps.interferogram, IWstr + '_top.xml')
        ifg_bot = os.path.join(inps.interferogram, IWstr + '_bottom.xml')
        if not os.path.exists(ifg_top) or not os.path.exists(ifg_bot):
            print('Skipping overlap_withDEM for {}: overlap interferogram files missing (1-burst stack). {}'.format(IWstr, inps.interferogram))
            continue
