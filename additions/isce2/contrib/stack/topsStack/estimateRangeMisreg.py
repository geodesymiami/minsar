...
def main(iargs=None):
    ...
    rangeOffsets = []
    snr = []
    # Last swath in swathList may be skipped (empty overlap bursts) after referenceTop
    # was reassigned for that swath — do not use referenceTop after the loop for scaling.
    referenceRangePixelSize = None
    ...
    for swath in swathList:
        ...
        # MinSAR 1-burst: overlap products have no bursts; skip this swath
        if not referenceTop.bursts or not secondaryTop.bursts:
            print('Skipping estimateRangeMisreg for IW{}: overlap has no bursts (1-burst stack).'.format(swath))
            continue

        referenceRangePixelSize = referenceTop.bursts[0].rangePixelSize
    ...
    # MinSAR 1-burst: no overlap bursts; write dummy range misreg and exit
    if not rangeOffsets:
        outputDir = os.path.dirname(inps.output)
        os.makedirs(outputDir, exist_ok=True)
        with open(inps.output, 'w') as f:
            f.write('median : 0.0\nmean : 0.0\nstd : 0.0\nsnr threshold : '+str(inps.offsetSNRThreshold)+'\nnumber of coherent points : 0\n')
        print('estimateRangeMisreg: no overlap bursts (1-burst stack); wrote dummy range misreg (0.0).')
        return
    ...
    if referenceRangePixelSize is None:
        raise RuntimeError('estimateRangeMisreg: referenceRangePixelSize unset despite non-empty range offsets')
    medianval = medianval * referenceRangePixelSize
    meanval = meanval * referenceRangePixelSize
    stdval = stdval * referenceRangePixelSize
