...
description: >-
  ...
  new script (no test_*.bash or test_*.py by default); do not always edit
  architecture_docs (merge conflicts). Use when planning, implementing, or
  testing MinSAR changes, especially new utils/scripts, PLAN.md, test_*.bash,
  test_*.py, or architecture_docs updates.
---
...
- **Do not** add `tests/test_<script>.bash` or `minsar/*/tests/test_<script>.py` for every new utility script by default.
...
- Prefer manual smoke checks (`--help`, a quick dry-run) over new test files for one-off admin/utils tools (e.g. `clean_dir.bash`, `generate_sweets_config.py`).
